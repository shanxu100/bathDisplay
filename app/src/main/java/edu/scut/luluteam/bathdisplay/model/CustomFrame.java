...
public class CustomFrame {
    ...
    public int getIntLength() {
        if (length != null) {
            return ByteUtil.toInteger(length[0], length[1]);
        } else {
            return 0;
        }
    }
    ...
    public static class Builder {
        ...
        private static CustomFrame decode(byte[] bytes, int readIndex) {
            ...
            //校验长度
            int dLength = ByteUtil.toInteger(bytes[readIndex], bytes[readIndex + 1]);
...
//        int EOFIndex = 1 + 2 + dLength + 2;
        ...
        private static void printErrorFrame(String title, byte[] bytes) {
            Log.e(TAG, " Frame Decoder:  " + title + " --frame: " + ByteUtil.byte2hex(bytes));
        }
    }
    ...
    public String toString() {
        return "CustomFrame_HEX: {" +
                "header:" + ByteUtil.byte2hex(new byte[]{header})
                + "  length:" + ByteUtil.byte2hex(length)
                + "  data: " + ByteUtil.byte2hex(data)
                + "  crc16:" + ByteUtil.byte2hex(crc16)
                + "  tail:" + ByteUtil.byte2hex(new byte[]{tail})
                + '}';
    }
}
