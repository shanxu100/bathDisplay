...
//         }
    ...
    private CustomFrame dequeueWholeFrame() {
        final Object[] items = this.items;
        int count = this.count;
        while (CustomFrame.SOF_FRAME != (byte) items[takeIndex]) {
            dequeue();
            count = this.count;
        }
        ...
        try {
            ...
            System.out.println("sof takeIndex:" + takeIndex);
            byte sof = (byte) items[takeIndex];
            //数据长度
            byte[] length = new byte[CustomFrame.LENGTH_SIZE];
            length[0] = (byte) items[takeIndex + 1 == items.length ? 0 : takeIndex + 1];
            length[1] = (byte) items[takeIndex + 2 >= items.length ? takeIndex + 2 - items.length : takeIndex + 2];

            //数据
            int lengthData = ByteUtil.toInteger(length[0], length[1]);
            byte[] data = new byte[lengthData];
            if (count >= lengthData + 6) {
                //如果items的长度满足一个完整的帧长
                dequeue();
                dequeue();
                dequeue();
            ...
            byte[] crc16 = new byte[CustomFrame.CRC16_SIZE];
            crc16[0] = (Byte) dequeue();
            crc16[1] = (Byte) dequeue();

            byte eof = (Byte) dequeue();
            byte[] result = new byte[lengthData + 6];
            int pos = 0;
            result[pos++] = sof;
            System.arraycopy(length, 0, result, pos, 2);
            pos += 2;
            System.arraycopy(data, 0, result, pos, lengthData);
            pos += lengthData;
            System.arraycopy(crc16, 0, result, pos, 2);
            pos += 2;
            result[pos] = eof;
            CustomFrame customFrame = CustomFrame.Builder.decode(result);
            return customFrame;
        } finally {
            System.out.println("error takeIndex" + takeIndex);
        }

    }

    public CustomFrame takeWholeFrame() throws InterruptedException {
        final ReentrantLock lock = this.lock;
        lock.lockInterruptibly();
        try {
            while (count <= 6)
                notEmpty.await();
            return dequeueWholeFrame();
        } finally {
            lock.unlock();
        }
    }

    public void putAll(E[] e) throws InterruptedException {
        Objects.requireNonNull(e);
        final ReentrantLock lock = this.lock;
        lock.lockInterruptibly();
        try {
            while (count + e.length >= items.length)
                notFull.await();
            enqueueAll(e);
        } finally {
            lock.unlock();
